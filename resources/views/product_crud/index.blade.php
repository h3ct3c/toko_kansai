...
@section('page_title', content: 'products')
...
<div class="bg-white shadow-xl rounded-xl overflow-hidden">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
        ...
        class="mt-4 sm:mt-0 bg-blue-900 hover:bg-blue-800 text-white font-medium px-6 py-2 rounded-lg shadow-md transfrom hover:scale-[1.02] transition duration-300">
    ...
    {{-- Card Kontainer Tabel (menggunakan rounded-xl, shadow-xl dari code lama) --}}
    <div class="bg-white shadow-xl rounded-xl border border-gray-200 overflow-hidden">
        
        <div class="overflow-x-auto">
            <table class="min-w-full text-left">  
                {{-- Header Tabel (Style header dari gambar UI baru) --}}
                <thead class="bg-white border-b border-gray-100">
                    <tr>
                        <th class="px-6 py-4 w-4">
                            <input type="checkbox" class="rounded text-blue-600 focus:ring-blue-500 border-gray-300">
                        </th>
                        {{-- Kolom sesuai data Anda sebelumnya --}}
                ...
                @forelse ($products as $product)
                    <tr class="hover:bg-gray-50 transition duration-150">
                        ...
                        {{-- Product Name (Gaya 2 baris dari gambar UI) --}}
                        <td class="px-6 py-3">
                        ...
                        {{-- Actions (Tombol Edit dan Delete dengan gaya dari gambar UI) --}}
                        <td class="px-6 py-3 text-right">
                            <div class="flex justify-end space-x-2">
                                
                                {{-- Tombol Edit item --}}
                                <a href="{{ route('product_crud.edit', $product) }}"
                                class="inline-flex items-center h-8 px-3 py-2 text-xs font-medium text-gray-700 bg-gray-100 border border-gray-300 rounded-lg hover:bg-gray-100 shadow-sm transfrom hover:scale-[1.02] transition duration-300">
                                ...
                                <form action="{{ route('product_crud.destroy', $product) }}" method="POST" onsubmit="return confirm('❗ Yakin hapus produk {{ $product->name }}? Aksi ini tidak dapat dibatalkan.')">
                                    ...
                                    @method('DELETE')
                                    <button type="submit"
                                            class="inline-flex items-center px-3 py-2 text-xs font-medium text-white bg-red-500 rounded-lg hover:bg-red-600 shadow-sm transfrom hover:scale-[1.02] transition duration-300">
